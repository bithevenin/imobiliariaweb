...
<head>
    ...
    <style>
        ...
        .preview-item .edit-existing-btn {
            position: absolute;
            top: 2px;
            left: 2px;
...
<body class="bg-light">
        ...
        <div class="d-flex">
            <nav class="sidebar" id="sidebar">
                ...
                <div class="sidebar-menu">
                    ...
                    <a href="<?php echo SITE_URL; ?>/admin/analytics.php"><i class="fas fa-chart-line me-2"></i>Analíticas</a>
                    <a href="<?php echo SITE_URL; ?>/admin/properties-manage.php"><i
                            class="fas fa-building me-2"></i>Propiedades</a>
                    <a href="<?php echo SITE_URL; ?>/admin/property-form.php" class="active"><i
                            class="fas fa-plus-circle me-2"></i>Nueva Propiedad</a>
                    <a href="<?php echo SITE_URL; ?>/admin/messages.php">
                        <i class="fas fa-envelope me-2"></i>Mensajes
                        <?php if ($unread_count > 0): ?><span
                                class="badge bg-danger ms-2"><?php echo $unread_count; ?></span><?php endif; ?>
                    </a>
                    <a href="<?php echo SITE_URL; ?>/index.php" target="_blank"><i
                            class="fas fa-external-link-alt me-2"></i>Ver Sitio Web</a>
                    <hr class="mx-3 bg-white-50">
                    <a href="<?php echo SITE_URL; ?>/admin/logout.php" class="text-danger"><i
            ...
            <main class="main-content p-3 p-md-4">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <p class="text-muted mb-0 small">
                                <?php echo $property_id ? 'Actualiza los datos de la propiedad' : 'Completa los campos para publicar'; ?>
                            </p>
                        </div>
                ...
                <?php if ($error): ?>
                    <div class="alert alert-danger shadow-sm border-0 small mb-3"><i
                            class="fas fa-exclamation-triangle me-2"></i><?php echo $error; ?></div><?php endif; ?>
                <?php if ($success): ?>
                    <div class="alert alert-success shadow-sm border-0 small mb-3"><i
                            class="fas fa-check-circle me-2"></i><?php echo $success; ?></div><?php endif; ?>
                ...
                <div class="card shadow-sm border-0">
                    <div class="card-body p-3 p-md-4">
                        <form id="property-form" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="row g-3">
                                <div class="col-md-9"><label class="form-label small fw-bold">Título *</label><input
                                        type="text" class="form-control form-control-sm" name="title"
                                        value="<?php echo $property['title'] ?? ''; ?>" required></div>
                                <div class="col-md-3">
                                    <label class="form-label small fw-bold">Tipo *</label>
                                    <select class="form-select form-select-sm" name="type" required>
                                        <option value="">Seleccionar...</option>
                                        <?php $types = ['Casa', 'Apartamento', 'Villa', 'Solar', 'Oficina', 'Local', 'Penthouse', 'Terreno'];
                                        foreach ($types as $t) {
                                            $s = ($property['type'] ?? '') === $t ? 'selected' : '';
                                ...
                                <div class="col-6 col-md-3"><label
                                        ...
                                        value="<?php echo $property['bedrooms'] ?? 0; ?>"></div>
                                <div class="col-6 col-md-3"><label class="form-label small fw-bold">Baños</label><input
                                        type="number" class="form-control form-control-sm" name="bathrooms"
                                        value="<?php echo $property['bathrooms'] ?? 0; ?>"></div>
                                <div class="col-6 col-md-3"><label class="form-label small fw-bold">Área
                                ...
                                <div class="col-6 col-md-4">
                                    ...
                                    <select class="form-select form-select-sm" name="status">
                                        <?php $stats = ['Disponible', 'Vendida', 'Reservada'];
                                        foreach ($stats as $st) {
                                            $s = ($property['status'] ?? '') === $st ? 'selected' : '';
                                ...
                                <div class="col-6 col-md-4">
                                    ...
                                    <div class="form-check form-switch mt-1"><input class="form-check-input"
                                            type="checkbox" name="featured" <?php echo ($property['featured'] ?? 0) ? 'checked' : ''; ?>></div>
                                </div>
                                    ...
                                    <div id="main-preview" class="preview-container">
                                        <?php if (!empty($property['image_main'])): ?>
                                            <div class="preview-item" id="main-c"><img
                                                    src="<?php echo $property['image_main']; ?>"><button type="button"
                                                    class="remove-btn"
                                                    onclick="removeExistingImage('<?php echo $property['image_main']; ?>', 'main-c')"><i
                                                        class="fas fa-trash"></i></button></div><?php endif; ?>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    ...
                                    <div id="gallery-preview" class="preview-container">
                                        <?php if (!empty($property['image_gallery'])):
                                            ...
                                            foreach ($gal as $i => $u): ?>
                                                <div class="preview-item" id="gal-<?php echo $i; ?>"><img
                                                        src="<?php echo $u; ?>"><button type="button" class="remove-btn"
                                                        onclick="removeExistingImage('<?php echo $u; ?>', 'gal-<?php echo $i; ?>')"><i
        ...
        <div class="modal-dialog modal-lg border-0 shadow">
            <div class="modal-content border-0">
                <div class="modal-header bg-dark text-white border-0">
                    <h6 class="modal-title h6">Elegir Ubicación</h6><button type="button"
                        class="btn-close btn-close-white" data-bs-dismiss="modal"></button>
                </div>
                <div class="modal-body p-0">
                    <div id="map-selector"></div>
                </div>
                <div class="modal-footer border-0"><button type="button" class="btn btn-primary btn-sm px-4"
                        id="confirm-location" data-bs-dismiss="modal">Confirmar</button></div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="cropperModal" data-bs-backdrop="static" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg border-0 shadow">
            <div class="modal-content border-0">
                <div class="modal-header bg-dark text-white border-0">
                    <h6 class="modal-title">Editar Imagen</h6><button type="button" class="btn-close btn-close-white"
                ...
                <div class="modal-footer border-0"><button type="button" class="btn btn-primary btn-sm px-4"
                        id="save-crop">Aplicar y Optimizar</button></div>
            </div>
        </div>
    </div>
