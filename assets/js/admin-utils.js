...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateSizeInfo() {
        ...
        canvas.toBlob((blob) => {
            ...
            document.getElementById('current-img-size').textContent = `Tamaño: ${sizeText}`;
        }, 'image/jpeg', 0.7);
    }
    ...
    function processCroppedImage(base64, blob) {
        ...
        document.getElementById('property-form').appendChild(hiddenInput);

        // Preview con badge de tamaño
        ...
        if (currentCropTarget === 'main') {
            // Eliminar inputs previos de main si existen
            const oldMain = document.querySelectorAll('input[name="cropped_main"]');
            oldMain.forEach(el => el.remove());
            container.innerHTML = '';
        }

        // Si veníamos de editar una imagen existente, marcarla para borrar
...
/**
 * Cargar imagen existente para edición
 */
function editExistingImage(url, containerId, target) {
    ...
    fetch(url)
        ...
        .catch(err => {
            alert('Error al cargar la imagen para edición. Puede ser un problema de CORS.');
            console.error(err);
        });
}

// Modificar saveCropBtn listener para ejecutar la eliminación pendiente
// Buscamos el listener previo o lo sobreescribimos. En mi admin-utils.js ya está definido.
// Lo actualizaré en el bloque processCroppedImage para que use pendingDeletionUrl.
